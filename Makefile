...
# если подключаем доп. библиотеку прописываем путь где лежат ее хедеры, пример ниже
...
# $(SDL_FLAGS) $(SDL) в случае компиляции доп. библиотек вставляем после inc_dir флаги и путь 
...
#(SDL) в случае компиляции доп. библиотек вставляем после inc_dir  путь 
